...
# Coded by ins3ct and Zirou
#
# Coded for Priv8 Server
#
# Thanks hc0d3r, HyOgA, idz, chk_, vL, psycco, Joomla_ and all the #NOSAFE family.
#
# Let's Rock! ;D
#
#
...
class NoSafe:
	...
	def FTPConnect(self, banner, canal, ftp_host, ftp_user, ftp_pass):
		...
		try:
			...
			self.SendMsg(canal, banner + '0,1[4 FTP 0] 15{}4 => 4CONEXÃO RECUSADA '.format(str(ftp_host)))
		...
		if len(command) == 1:
			if canal != self.nick:
				if command[0] == 'help' or command[0] == 'ajuda':
					self.SendMsg(canal, banner + '0,1[4 HELP 0] 15Comandos disponíveis 4->14 http://pastebin.com/PrzPuXe6')
			...
			if command[0] == 'bitcoin':
				try:
					...
					#self.SendMsg(canal, banner + '0,1[4 BITCOIN 0] Maximum Price USD:4 {:.6} '.format(str(btc_data['btc_usd']['high'])))
					#self.SendMsg(canal, banner + '0,1[4 BITCOIN 0] Minimum Price USD:4 {:.6} '.format(str(btc_data['btc_usd']['low'])))
					#self.SendMsg(canal, banner + '0,1[4 BITCOIN 0] Buy Price USD:4 {:.6} '.format(str(btc_data['btc_usd']['buy'])))
					#self.SendMsg(canal, banner + '0,1[4 BITCOIN 0] Sell Price USD:4 {:.6} '.format(str(btc_data['btc_usd']['sell'])))
					#self.SendMsg(canal, banner + '0,1[4 BITCOIN 0] Quotation:4 {} '.format(str(time.strftime('%d %h %Y - %H:%M'))))
				except Exception, e:
					print '[BITCOIN] ERRO:', e

			# TERMINAR DEPOIS
			#
			# if command[0] == 'quotation':
			# 	self.SendMsg(canal, 'Recebendo parametros do site.... aguarde....')
			# 	r = requests.get('http://www.cryptocurrencychart.com/')
			# 	soup = BeautifulSoup.BeautifulSoup(r.content)
			# 	title = soup.table.findAll('td',{'class':'name'})[0].text
			# 	self.SendMsg(canal, '>>> ' + str(title))
			...
			if canal != self.nick:
				...
				if command[0] == 'range':
					try:
						port_host = command[1]
						port_port_num = command[2]
						port_port = command[3]
						port_list_scan = []
						
						try:
							port_host_split = port_host.split('.')[3]

							for x in range(int(port_host_split), (int(port_port_num) + 1)):
				...
				if command[0] == 'portscan':
					...
					port_list = [1,5,7,18,20,21,22,23,25,29,37,42,
								...
								1080,3389,6667,8002,34567,34599]
					try:
				...
				if command[0] == 'hex':
					try:
						...
						#self.SendMsg(canal, banner + '0,5[4HEX0] HEX => STRING:4 {}'.format(str(hex_hex)))
				...
				if command[0] == 'dehex':
					try:
						...
						try:
							...
							self.SendMsg(canal, banner + '0,1[4 HEX 0]14 STRING 4=>14 HEX:15 {}'.format(str(hex_decoded.lstrip())))
						except:
	...
	def run(self):
		...
		while self.close == False:
			...
			if str(self.data).find(str(base64.b16decode('53656E686120616365697461202D'))) != -1:
				self.s.send('JOIN {}\r\n'.format(self.channel))
...
if __name__ == '__main__':
	...
	ajoin = ['#nosafe', '#python']#, '#priv8', '#jamaica', '#inurlbrasil', '#brasil', '#protowave'] # Canais secundários, .sendall enviará mensagem para esses canais.
	admin = ['ins3ct', 'Zirou'] # Nicks para acessos à funções especiais do bot
